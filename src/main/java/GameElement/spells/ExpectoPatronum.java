...
public class ExpectoPatronum extends Spell {
    ...
    public void cast(Wizard wizard, Character target) {
        ...
        if (target instanceof Detraqueur detraqueur) {
            castOnDetraqueur(wizard, detraqueur);
        } else {
    ...
    private void castOnDetraqueur(Wizard wizard, Detraqueur detraqueur) {
        int chanceOfSuccess = 0;
        if (detraqueur.getDistance() <= 5) {
            System.out.println("Vous utilisez Expecto Patronum pour mettre les détraqueurs en fuite !");
            chanceOfSuccess = getChanceOfSuccess(wizard, detraqueur);
            System.out.println("Vous êtes à " + detraqueur.getDistance() + " mètres des détraqueurs. Votre chance de réussite est de " + chanceOfSuccess + "%.");
            ...
            System.out.println("Vous êtes à " + detraqueur.getDistance() + " mètres des détraqueurs. Votre chance de réussite est de " + chanceOfSuccess + "%., si vous aviez mieux écouter le cours vous sauriez que pour utiliser votre patronus et qu'il effraie quelqu'un \nil faut être à 5 mètres ou moins pour que le sort est une chance de réussir et que plus vous êtes proche plus vous augmenter vos chances.");
        }
    }
}
